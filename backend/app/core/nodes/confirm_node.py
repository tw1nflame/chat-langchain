...
def confirm_plan(state: Dict[str, Any]):
    ...
    prompt = (
        ...
        f"Вопрос пользователя: {state.get('question', '')}\n\n"
        ...
        "Если среди шагов есть генерация SQL (например, при запросах NWC), укажите, что будет сформирован SQL-запрос и какие целевые модели/пайплайны будут использованы для статей; "
        "если присутствует шаг выполнения SQL — укажите, что запрос будет выполнен и реальные данные будут извлечены; "
        "Если запрос носит аналитический характер (например, 'проанализируй прогноз'), укажите, что будут извлечены реальные данные за последний год по целевой модели(ям), на их основе будут построены графики, и будет подготовлен краткий итоговый отчёт с выводами; "
        "если есть шаг суммаризации — укажите, что в конце будет сформирован краткий итоговый отчёт по результатам.\n"
        "Кратко (1-2 предложения) опишите на русском языке, что будет сделано, и задайте один вопрос для подтверждения, например: 'Продолжить выполнение этих шагов?'.\n"
